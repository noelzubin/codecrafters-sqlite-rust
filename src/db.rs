...
pub fn parse_schemas(database: &[u8], number_of_cells: u16) -> Result<Vec<Schema>> {
    trace!("getting {} cell pointers from 108...", number_of_cells);
    let cell_pointers = parse_cell_pointers(&database[108..], number_of_cells);
    trace!("cell pointers: {:?}", &cell_pointers);
...
pub fn parse_btree_leaf_cell_content(
    ...
    trace!("getting record from cell pointer {}", cell_pointer);
    ...
    trace!("payloadsize: {}, offset: {}", _payload_size, offset);
    let (_rowid, read_bytes) = parse_varint(&stream[offset..]); // integer key (rowid).
    trace!("_rowid: {}", _rowid);

    trace!("parsing record at offset {}", offset + read_bytes);
...
fn parse_page(
    ...
    trace!("start_index: {}", &start_index);
    // Get the index page
    let page_header = get_page_header(&database[start_index..])?;
    warn!("page_header: {:#?}", &page_header);
    ...
    if page_header.page_type == BTreePage::InteriorIndex {
        ...
        for cell_pointer in cell_pointers.iter() {
            ...
            trace!("left child pointer: {}", &left_child_pointer);
            ...
            trace!("payload size: {}, offset: {}", payload_size, offset);
            ...
            if value <= &key {
                ...
                if value == key {
                    ...
                    warn!("found interior value {} ", value);
                    warn!("got result {:?}", rowid);
                } else {
                    found_bigger = true;
                }

                warn!(
                    "Interior value '{}' <= '{}', going to page {}",
                    value, key, left_child_pointer
                );
        ...
        warn!(
            "Interior value '{}' bigger than all values. going to page  {}",
            value, right_most_pointer
        );
    ...
    if page_header.page_type == BTreePage::LeafIndex {
        ...
        for cell_pointer in &cell_pointers {
            ...
            if key == value.as_bytes() {
                ...
                row_collector.push(rowid as usize);
                warn!("Found leaf value '{value}'; rowid:{:?}", rowid);
            }
        }
    }
...
fn get_them_records(database: &Vec<u8>, page_size: usize, page_number: usize) -> Vec<Vec<String>> {
    ...
    trace!(
        "page_size:{}, page_number:{} , start_index:{}",
        page_size,
        page_number,
        start_index
    );
